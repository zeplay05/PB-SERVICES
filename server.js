...
async function probeSupabase() {
    try {
        console.log("\x1b[36m[Supabase] Probing database connection (table: 'slips')...\x1b[0m");
        ...
        if (res.ok) {
            console.log("\x1b[32m[Supabase] Connection Successful! Table 'slips' is ready in your database.\x1b[0m");
            detectedTables = ['slips'];
        } else if (res.status === 404) {
            console.log("\x1b[33m[Supabase Info] Connected to Supabase, but table 'slips' does not exist yet (404).\x1b[0m");
            console.log("\x1b[31m[Supabase Warning] Please run the SQL command in the SQL Editor to create the 'slips' table.\x1b[0m");
            detectedTables = [];
        } else {
            console.error("\x1b[31m[Supabase Error] Connection probe failed with status:\x1b[0m", res.status, res.statusText);
            const text = await res.text();
            console.error("Details:", text);
            detectedTables = [];
        }
    } catch (err) {
        console.error("\x1b[31m[Supabase Error] Connection failed:\x1b[0m", err.message);
        detectedTables = [];
    }
}
...
app.post('/api/verify-slip', upload.single('slip'), async (req, res) => {
    try {
        ...
        const base64Raw = file.buffer.toString('base64');
        const base64DataUrl = `data:${file.mimetype};base64,${base64Raw}`;
        ...
        // Call Slip2Go verification API
        try {
            console.log(`Calling Slip2Go API endpoint (User: ${username}, Tester: ${isTester})...`);
            const slip2goResponse = await fetch("https://api.slip2go.com/api/verify-slip/qr-image/info", {
                method: "POST",
                headers: {
                    "Authorization": "Bearer CwHcJy4qbtGCRgw3McQMHdAUgFTbpoXb2tKTfrPm2qM=",
        ...
        if (detectedTables.length > 0 && !detectedTables.includes('slips')) {
            targetTable = detectedTables.includes('transactions') ? 'transactions' : (detectedTables[0] || 'slips');
        }
        ...
        if (!isTester) {
            try {
                ...
                const dupCheckResponse = await fetch(`${SUPABASE_URL}${targetTable}?ref_number=eq.${refNumber}`, {
                    method: "GET",
                    headers: {
                        "apikey": SUPABASE_KEY,
                        "Authorization": `Bearer ${SUPABASE_KEY}`
        ...
        try {
            ...
            if (dbResponse.ok) {
                ...
                console.log("\x1b[32m[Supabase] Row successfully saved to database!\x1b[0m", insertedData);
                savedToDb = true;
            } else {
                const errText = await dbResponse.text();
                console.error(`\x1b[31m[Supabase Database Error]\x1b[0m Status: ${dbResponse.status}, Details:`, errText);
                dbError = `Database status ${dbResponse.status}: ${errText}`;
            }
        } catch (dbErr) {
            console.error(`\x1b[31m[Supabase Connection Error] Failed to write:\x1b[0m`, dbErr.message);
...
app.listen(PORT, async () => {
    console.log(`\x1b[32m[Server] Express Backend is running on http://localhost:${PORT}\x1b[0m`);
    ...
    try {
        ...
        if (fs.existsSync(qrSrc)) {
            ...
            console.log("\x1b[32m[Server] Successfully copied real QR code to public/promptpay-qr.png\x1b[0m");
        } else {
            console.warn("\x1b[33m[Server Warning] Real QR code source file not found at:\x1b[0m", qrSrc);
        }
    } catch (err) {
